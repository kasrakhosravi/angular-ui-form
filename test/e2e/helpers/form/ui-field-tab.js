...
module.exports = {
    ...
    setData: function(pageObject, data) {
        return pageObject.buildChildrenPageObjects().then(function (children) {
            ...
            return headers.count().then(function (headersCount) {
                if (headersCount > 0) {
    ...
    clearData: function(pageObject) {
        var formElement = pageObject.getElement(),
            headers = formElement.all(by.xpath('./ui-field-row/div/div/ng-transclude/div/ul/li'));
        ...
        return headers.count().then(function (headersCount) {
            if (headersCount > 0) {
                return headers
                    ...
                    .then(function (headersArray) {
                        return util.walkByPromise(headersArray, function (header) {
                            return headers.get(header.index).click().then(function () {
                                return DefaultHelper.clearData(
                                    new FormPageObject(
                                        formElement
                                            .all(by.xpath('./ui-field-row/div/div/ng-transclude/div/div/div'))
                                            .get(header.index)
